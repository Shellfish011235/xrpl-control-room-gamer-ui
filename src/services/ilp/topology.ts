...
export const INITIAL_CONNECTORS: Connector[] = [
  ...
  // XRPL <-> XRPL EVM Sidechain (Official XRPL bridge)
  {
    ...
    trust_score: 0.85,
    trust_reason: 'HIGH TRUST for mainnet lock/mint; caveat for sidechain: Official XRPL sidechain with a bridge that locks XRP on mainnet and mints 1:1 on the sidechain. The sidechain does not use XRP consensus to settle and does not yet validate fraud proofs (pending ZKP amendments). You rely on trusted third parties that bridged assets are represented accurately on the sidechain. Blockchain-to-blockchain direct signing is the gold standard for cross-chain interop.',
    latency_ms: 5000,
    settlement: 'bridge',
    risk_flags: ['bridge_risk'],
    claims: [
      {
        ...
        statement: 'Official XRPL EVM Sidechain bridge - locks XRP on mainnet, mints on sidechain. Sidechain settlement does not use XRP consensus; no fraud proofs yet (ZKP amendments pending). Trusted third parties for bridged asset representation.',
  ...
  // XRPL EVM Sidechain <-> Ethereum (via standard EVM bridges)
  {
    ...
    trust_score: 0.7,
    trust_reason: 'MEDIUM TRUST: Cross-chain bridges to Ethereum carry inherent risk. The XRPL EVM sidechain does not use XRP consensus for settlement and does not yet validate fraud proofs (pending ZKP amendments); you rely on trusted third parties for bridged asset representation. Major bridge hacks include Ronin ($625M), Wormhole ($320M), Nomad ($190M). Smart contract vulnerabilities are a concern.',
    latency_ms: 15000,
    settlement: 'bridge',
